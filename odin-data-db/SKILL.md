...
```bash
scripts/odin-db-query --check-setup
```

If setup is not complete, guide the user through setup:
1. Direct them to read: `references/setup.md`
2. Help them create the config file at `~/.config/odin-db/config.json`
...
scripts/odin-db-query --env <ENV> --list-dbs
```

## Available Commands

Script: `scripts/odin-db-query`

### Query Database

```bash
# Query default database
scripts/odin-db-query --env <ENV> --sql "SELECT * FROM users WHERE id = 123"

# Query specific database (env:db format)
scripts/odin-db-query --env <ENV>:<DB> --sql "SELECT * FROM events LIMIT 10"

# With output format
scripts/odin-db-query --env <ENV> --sql "SELECT ..." --format csv
scripts/odin-db-query --env <ENV> --sql "SELECT ..." --format json
```
...
```bash
scripts/odin-db-query --env <ENV> --tunnel start
scripts/odin-db-query --env <ENV> --tunnel status
scripts/odin-db-query --env <ENV> --tunnel stop   # optional - auto-closes after 10min idle
```
...
```bash
scripts/odin-db-query --env <ENV>:<DB> --sql "UPDATE users SET status = 'active' WHERE id = 123" --write
```
...
3. **Explore schema**:
   ...
   scripts/odin-db-query --env <ENV> --sql "\\dt"
   scripts/odin-db-query --env <ENV> --sql "\\d table_name"
   # Alternative using standard SQL (works with all --format options):
   scripts/odin-db-query --env <ENV> --sql "SELECT tablename FROM pg_tables WHERE schemaname = 'public'"

   # MySQL
   scripts/odin-db-query --env <ENV>:<DB> --sql "SHOW TABLES"
   scripts/odin-db-query --env <ENV>:<DB> --sql "DESCRIBE table_name"
   ```

4. **Investigate data**:
   ```bash
   scripts/odin-db-query --env <ENV> --sql "SELECT * FROM orders WHERE user_id = 123 ORDER BY created_at DESC LIMIT 10"
   ```

5. **Check query performance**:
   ```bash
   scripts/odin-db-query --env <ENV> --sql "EXPLAIN ANALYZE SELECT ..."
   ```
...
All connection details come from AWS Secrets Manager or the user's private config file (`~/.config/odin-db/config.json`), neither of which is shared or committed.
