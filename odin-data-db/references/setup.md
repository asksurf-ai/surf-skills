...
2. **Local config file** (`~/.config/odin-db/config.json`) — Fallback when AWS is unavailable.
...
```bash
mkdir -p ~/.config/odin-db
chmod 700 ~/.config/odin-db
```

## Step 2: Create Configuration File

```bash
touch ~/.config/odin-db/config.json
chmod 600 ~/.config/odin-db/config.json
```

Edit `~/.config/odin-db/config.json`:
...
✓ Config file exists: ~/.config/odin-db/config.json
...
scripts/odin-db-query --env stg --list-dbs

# Start tunnel for default database
scripts/odin-db-query --env stg --tunnel start

# Or start tunnel for specific database
scripts/odin-db-query --env stg:analytics --tunnel start

# Test query
scripts/odin-db-query --env stg --sql "SELECT 1"

# Query specific database
scripts/odin-db-query --env stg:analytics --sql "SELECT 1"
```
...
scripts/odin-db-query --env stg --sql "SELECT * FROM users LIMIT 5"

# Use specific database
scripts/odin-db-query --env stg:analytics --sql "SELECT * FROM events LIMIT 5"
scripts/odin-db-query --env stg:events --sql "SHOW TABLES"

# Tunnel management per database
scripts/odin-db-query --env stg:main --tunnel start
scripts/odin-db-query --env stg:analytics --tunnel start
```
...
- Check if tunnel is running: `scripts/odin-db-query --env stg --tunnel status`
...
```bash
scripts/odin-db-query --env stg --tunnel stop
```
