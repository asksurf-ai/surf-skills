...
surf-token search --query ethereum

# Step 2: Get full metadata (supply, links, categories, multi-chain deployments)
surf-token info --address 0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48 --chain eth

# Step 3: Check holder concentration — top 10 holders
surf-token holders --address 0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48 --chain eth --limit 10

# Step 4: See who is actively trading it
surf-token top-traders --address 0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48
```
...
surf-token metrics --asset btc --metric exchange_flow --window day --limit 7

# Step 2: Total exchange reserves over time
surf-token metrics --asset btc --metric exchange_reserve --window day --limit 7

# Step 3: Compare ETH flows
surf-token metrics --asset eth --metric exchange_flow --window day --limit 7
```
...
surf-token metrics --asset btc --metric etf_flow --type us-btc-spot --limit 7

# ETH spot ETF daily flows
surf-token metrics --asset eth --metric etf_flow --type us-eth-spot --limit 7
```
...
surf-token top --metric volume --limit 10

# Biggest gainers today
surf-token top --metric gainers --limit 10

# Biggest losers today
surf-token top --metric losers --limit 10

# Tokens with highest exchange inflows (potential sell pressure)
surf-token top --metric exchange_inflow --limit 10

# Tokens with highest exchange outflows (accumulation signals)
surf-token top --metric exchange_outflow --limit 10

# Top tokens by social mindshare
surf-token top --metric mindshare --limit 10
```
...
surf-token unlock --id arbitrum

# Check with specific timeframe
surf-token unlock --id optimism --timeframe 30d
```
...
surf-market price --ids usd-coin --vs usd
surf-market chart --id usd-coin --vs usd --days 30

# 3. Project fundamentals (use surf-project)
surf-project overview --id usd-coin

# 4. Recent news (use surf-news)
surf-news search --query "USDC"

# 5. Social sentiment (use surf-social)
surf-social search --query "USDC" --limit 5
```
